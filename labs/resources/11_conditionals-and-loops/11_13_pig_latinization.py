...
#this function takes one word

#function get one word into pig latin
...
pig_latin_text = "" #returned text
word = "" #word by word

for char in text:
    if char.isalpha():  # leave the punctuation alone (for now)
        word += char #add it to the word we are working on - and then add that word to the total text we want to reutrn
    else: #we've hit something which isn't a character
        if word:  # convert the word to pig latin
            pig_latin_text += to_pig_latin(word) #we are adding the converted character to the return text
            word = "" #reset it until we hit the next word, and then reset this
        pig_latin_text += char  # otherwise add the punctuation back

print(pig_latin_text[:800])  # print a preview of the translated text

    """_to translate the text word by word
            -> we define a blank word which we want to reset each time we hit a new word, when iterating through the text
            -> we iterate through a character in the text
            -> if that character is not a word, it's added to the return text
            -> we carry on building out the word until we don't hit anymore punctuation
            -> at the point we have a word, we perform the translation into Pig Latin on it
            -> and then we return the text
            -> the entire thing is going word by word, you iterate through the characters and if they are in the alphabet - then you add them onto the word until we hit punctuation
                -> then translate them, then don't
    """
